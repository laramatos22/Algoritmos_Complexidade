...
void ListMove(List *l, int newPos)
{
  ...
  { // move to an inner node
    ...
    int i;
    if (l->currentPos == -1 || newPos < l->currentPos) // nao é valido ou quer andar para tras
    {
      //comecar no inicio da lista
      l->current = l->head;
      l->currentPos = 0;
    }
...
int ListSearch(List *l, const void *p)
{
  ...
  struct _ListNode *currentNode = l->head; // a pesquisa começa no inicio da lista (head)

  for (int i = 0; i < lastElem; i++)
  {

    int comp = l->compare(currentNode->item, p); // começa por fazer comparacao de items

    // If search succeeds, return 0 and move the current node to the found element.
    if (comp == 0)
    {
      l->current = currentNode; // o currentNode passa a ser o elemento encontrado
      l->currentPos = i;
      return 0;
    }
...
void *ListRemoveCurrent(List *l)
{
  ...
  else
  {
    ...
    struct _ListNode *previous = l->head;
    int i;
    for (i = 0; i < l->currentPos - 1; i++)
    {
