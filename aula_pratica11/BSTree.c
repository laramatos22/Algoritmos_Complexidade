...
// A: Uma vez que primeiro destroi os filhos do lado esquerdo, depois abate os filhos do lado direito e, por ultimo, destroi-se
// o proprio nó
...
static int _treeGetHeightREC(const struct _BSTreeNode* root) {
  ...
  if (root == NULL)
  {
    ...
    if (root->left != NULL)
    {
      esquerda = _treeGetHeightREC(root->left);
    }

    if (root->right)
    {
      direita = _treeGetHeightREC(root->right);
    }
    
    altura = (direita >= esquerda ? direita : esquerda) + 1;
...
// A: Sim, visto que a fila é criada desde o elemento mais pequeno para o maior elemento.
